...
import com.example.spring.Message;
import com.example.spring.MessageRepository;
...
class MessageStepDefinitions {
    ...
    MessageStepDefinitions(MessageRepository messageRepository, UserStepDefinitions userStepDefinitions) {
        this.messageRepository = messageRepository;
        this.userStepDefinitions = userStepDefinitions;
    }

    @Given("the user has posted the message {string}")
    void the_user_has_posted_the_message(String content) {
        User user = userStepDefinitions.getCurrentUser();
        messageRepository.save(new Message(user, content));
    }

    @Given("a User has posted the following messages:")
    void a_user_has_posted_the_following_messages(List<Message> messages) {
        User user = userStepDefinitions.getCurrentUser();
